...
5. `js/main.js` TypeScript modüllerine birebir port edildi (Sprint 2'de galeri
   genişletme + lightbox dahil tamamlandı).
6. *(Sprint 2)* Erişilebilirlik eklentileri: lightbox markup'ına
   `role="dialog"`/`aria-modal`/`aria-hidden` ve buton `aria-label`'ları;
   galeri kartlarına çalışma zamanında `role`/`tabindex`/`aria-label`;
   `src/main/a11y.css` focus-visible stilleri; focus yönetimi
   (`focusWhenVisible`). Ayrıntı: `docs/ACCESSIBILITY.md`. Fare ile
   kullanımda ve screenshot parity'de görsel etkisi yoktur.
7. *(Sprint 3)* Ana sayfadaki kamera tam ekran linkleri kaynakta mutlak
   `https://sucullukoyu.com/canli-kamera.html` iken clone'da kendi rotasına
   (`/canli-kamera.html`) çevrildi — clone kendi kamera duvarına gitmelidir;
   piksel parity etkilenmez.
8. *(Sprint 3)* Kamera duvarı davranışları kaynak kusurları düzeltilerek
   yeniden yazıldı (görsel sonuç birebir): timer leak yok, aktif sayaç state
   set'inden türetilir, sınırsız agresif reconnect yerine backoff+jitter,
   endpoint config/env üzerinden. `?cam=mock-*` query test modları eklendi
   (yalnızca whitelist; bilinmeyen değer disabled'a düşer; ağ isteği üretmez).
   Ayrıntı: `docs/CAMERA_PLAYER_ARCHITECTURE.md`.
9. *(Sprint 3)* **Ses butonu görünür metin tutarsızlığı (kaynakta doğrulandı):**
   kaynak `btn.textContent = isMuted ? '🔇 SES KAPAT' : '🔊 SES AÇ'` kullanır;
   yani ses KAPALIYKEN "SES KAPAT" yazar ve tıklama sesi AÇAR. Görünür metin
   birebir korunmuştur; yalnızca `aria-label` gerçek aksiyonu anlatır.
